...
export class BybitExchange implements Exchanger {
  ...
  async fetchTotalBalance(): Promise<{ [k: string]: number }> {
    const [walletBalance, earnBalance, advanceEarnBalance] = await Promise.all([
      this.fetchWalletBalance(),
      this.fetchEarnBalance().catch((e) => {
        console.error("Fetch Bybit earn balance failed:", e);
        return {};
      }),
      this.fetchAdvanceEarnBalance().catch((e) => {
        console.error("Fetch Bybit advance earn balance failed:", e);
        return {};
      }),
    ]);
  ...
  private async fetchAdvanceEarnBalance(): Promise<{ [k: string]: number }> {
    const path = "/earn/advance/position";
    const params = {
      category: "DualAssets",
    };
    const resp = await this.fetch<AdvanceEarnPositionResp>("GET", path, params);
  ...
  private async fetchEarnPosition(
    category: "FlexibleSaving" | "OnChain",
  ): Promise<EarnPositionResp> {
  ...
  private async fetch<T>(
    ...
    const queryString = params
      ? qs.stringify(params, { sort: this.alphabeticalSort })
      : "";
  ...
  private generateSignature(
    ...
    const message =
      timestamp + this.apiKey + recvWindow + queryString + requestBody;
