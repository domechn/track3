...
vi.mock("../../entities/transactions", () => ({
  TRANSACTION_HANDLER: { listTransactions: vi.fn() },
}));
...
function makeTx(over: Partial<any> = {}) {
  return {
    id: 1, assetID: 1, uuid: "u", assetType: "crypto" as AssetType, symbol: "BTC",
    wallet: "w", amount: 1, price: 100, txnType: "buy" as TransactionType,
    txnCreatedAt: "2026-01-01T00:00:00.000Z", createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z",
    ...over,
  };
}
...
describe("recent_transactions skill", () => {
  ...
  it("sorts by date desc and respects the limit", async () => {
    ...
    const result = await skill.execute("test", { limit: 2 }, undefined, undefined, {} as any);
  ...
  it("filters by symbol and assetType", async () => {
    ...
    vi.mocked(TRANSACTION_HANDLER.listTransactions).mockResolvedValue(txs);
    const result = await skill.execute("test", { symbol: "btc", assetType: "crypto" }, undefined, undefined, {} as any);
    const data = result.details.data as any;
