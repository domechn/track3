...
vi.mock("../../entities/transactions", () => ({
  TRANSACTION_HANDLER: { listTransactionsByDateRange: vi.fn() },
}));
...
describe("health_score skill", () => {
  ...
  it("computes HHI, top-3 weight, and emits a radar chart", async () => {
    const latest = { uuid: "u", createdAt: new Date("2026-06-01T00:00:00Z"), totalValue: 1000 };
    ...
    expect(result.details.chart?.type).toBe("radar");
    expect(result.details.chart?.labels).toHaveLength(5);
  });
  ...
  it("rewards cash positions and recent activity", async () => {
    ...
    const txs = Array.from({ length: 12 }, (_, i) => ({
      id: i, assetID: 1, uuid: "t", assetType: "crypto", symbol: "BTC", wallet: "w", amount: 0.01, price: 100,
      txnType: "buy", txnCreatedAt: new Date(`2026-05-${String(i + 1).padStart(2, "0")}T00:00:00Z`).toISOString(),
    }));
